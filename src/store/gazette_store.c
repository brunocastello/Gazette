 ...
 */
static void CacheFileName(const char *kind, const char *feedURL, Str255 out)
{
    ...
    /* "Feed XXXXXXXX" — recognisable in the Finder, and short enough that
       the 31-character HFS limit is nowhere near. Its articles' text is
       "Text XXXXXXXX", the same hash, so the two sort side by side. */
    out[0] = 13;
    out[1] = (unsigned char)kind[0]; out[2] = (unsigned char)kind[1];
    out[3] = (unsigned char)kind[2]; out[4] = (unsigned char)kind[3];
    out[5] = ' ';
    for (i = 0; i < 8; i++) {
...
static OSErr MakeCacheSpec(const char *kind, const char *feedURL,
                           Boolean createFolder, FSSpec *spec,
                           Boolean *existed)
{
    Str255 name;

    CacheFileName(kind, feedURL, name);
...
static GazetteStoreFile *KindCreate(const char *kind, const char *feedURL)
{
    FSSpec  spec;
    Boolean existed;
    OSErr   err;
    ...
    err = MakeCacheSpec(kind, feedURL, true, &spec, &existed);
    if (err != noErr) {
        return NULL;
    }
...
static GazetteStoreFile *KindOpen(const char *kind, const char *feedURL)
{
    FSSpec  spec;
    Boolean existed;
    short   refNum;
    ...
    if (MakeCacheSpec(kind, feedURL, false, &spec, &existed) != noErr || !existed) {
        return NULL;
    }
...
void GazetteStoreCacheDelete(const char *feedURL)
{
    ...
    if (MakeCacheSpec("Feed", feedURL, false, &spec, &existed) == noErr &&
        existed) {
        (void)FSpDelete(&spec);
    }
    if (MakeCacheSpec("Text", feedURL, false, &spec, &existed) == noErr &&
        existed) {
        (void)FSpDelete(&spec);
    }
}
