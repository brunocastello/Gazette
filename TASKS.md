...
> 2e. [x] Bruno, 2026-09-26: Win 95 and XP pass, Mac OS 9 tested; the sidebar's stale horizontal bar is gone (WM_SETFONT / rebuild when every label fits).
> **0.2.0 release (both builds), 2026-09-26:** LICENSE (MIT, third-party list as Gateway's); README for Windows (Bruno asked; Thanks credits stb_image and roytam1, MinGW-w64, NSIS, Wine); versions 0.2.0 in gazette_version.h, Gazette.r, Gazette.rc, and the User-Agent now from gazette_version.h; installer/gazette.nsi (C:\Gazette, Start menu, uninstaller asking before it removes Gazette.ini and Cache; no licence page); installer/README.TXT shared by installer, zip and floppy; windows.yml builds Setup.exe on every push and publishes the Windows assets on a tag beside build.yml's Mac ones; the Wine screenshot steps removed (Bruno). **Released 2026-09-26: tag v0.2.0 (b2ec414), runs 36266212585 Win32 / 36266212575 Carbon, five assets on https://github.com/brunocastello/Gazette/releases/tag/v0.2.0; repo About and topics updated.**
> **0.2.0 replaced, 2026-09-26 (tag now on f92bb2d, runs 36268946784 / 36268946799):** (1) Import/Export Feeds opened no dialog on Windows 95 -- MinGW-w64 always declares OPENFILENAMEA with the Windows 2000 fields (88 bytes) and 95/98/NT 4 refuse it; now OPENFILENAME_SIZE_VERSION_400A (76) with a compile-time check, and the store test drives both dialogs under Wine (xvfb). The Open dialog also lists *.opm (a Mac floppy's 8.3 name). (2) Photos per article up to 10 (Bruno typed 7; the cap was 3); default 3; the photo block sized 85 KB per picture asked for. Engine test proves the Preferences numbers survive a relaunch.
> **Next (after 0.2.0):** an install/upgrade/uninstall pass of Setup.exe on 86Box 95 and XP; W3/W4 leftovers below. Earlier note: If approved: installer, README and release as 0.2.0, versions bumped on both (gazette_version.h, Gazette.r, Gazette.rc).**
