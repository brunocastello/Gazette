...
- [~] Confirm the Google News decode on the Mac (built 2026-09-19, never seen working on real hardware or SheepShaver).
  Bruno, 2026-09-23: the NBC story "Defending Trump media ban, DOJ cites national security…" (Top Stories) shows something other than the article; Newsstand shows it right. The portable path is correct: `probe.yml` run 35880811279 decodes the Google link to the NBC URL and extracts the whole article (5,163 chars + lead photo) with Gazette's own C. What the Mac most likely shows is the fallback summary, which for Google News is a list of the same story from other outlets (NYT, Guardian, Yahoo, CNN). So a Mac-only step fails. The next thing needed is the status line ("Summary only - …") to know which step.
  Also: from Spain, Google sends the first GET through consent.google.com (3 redirects, ~2.3 KB heads, final page 583 KB with the two attributes at ~581 KB). That fits Gazette's limits (5 redirects, 8 KB heads, 4 KB paths), but CI in the US never sees that chain.
  Status line on the Mac: "Summary only - the connection failed while reading headers", which is `MacTLS_Read` < 0 after the handshake, on one of the four hosts. NBC speaks only TLS 1.2 (Akamai rejects every TLS 1.3 suite), so NBC goes through BearSSL, not Certainly's 1.3 code. From CI, BearSSL with Certainly's exact 1.2 settings fetches the page fine (`tls-probe.yml` run 35882027601). Next: debug build run 35882242314 (`GAZETTE_NET_DEBUG`) on the Mac, whose status line names the host, TLS version and both error codes.
- [x] `tls-probe.yml`: one https URL through the vendored BearSSL with Certainly's TLS 1.2 settings, printing BearSSL's verdict.
- [x] `build.yml` `net_debug` input; the debug detail now names host, TLS version, MacTLS_Error and BR_ERR_*.
- [x] `tests/host/probe` + `tools/probe.sh` + `.github/workflows/probe.yml`: run one link through the resolver and extractor on CI (`gh workflow run probe.yml -f link=…`); the probe is built in the host-test job so it cannot rot.
