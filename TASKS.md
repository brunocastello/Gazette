...
- [~] Google News JS-redirect article links: best-effort decode (NewsProxy technique) **or** clear status-line fallback to feed summary.
  Decode built (GET `rss/articles/<token>` → POST `batchexecute` → story URL). On CI the whole path gets the full NBC article (probe run 35880811279). On Bruno's Mac it fails: "Summary only - the connection failed while reading headers" — a TLS read error after the handshake, on one of four hosts (news.google.com, consent.google.com from Spain, batchexecute, www.nbcnews.com, which is TLS 1.2 only). BearSSL with Certainly's 1.2 settings fetches NBC fine from CI (tls-probe run 35882027601). Next: debug build run 35885762796 (artifact Gazette-carbon-ppc-macos9-NETDEBUG) on the Mac, to name the host and the error codes.
