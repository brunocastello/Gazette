...
- [x] File I/O / cache path for Windows (prefs + cache folders beside the exe or under AppData — pick one, document it).
  Beside `Gazette.exe` (as Gateway), falling back to `%APPDATA%\Gazette`, then `%USERPROFILE%\Gazette`; `Gazette.ini` (was `Gazette Preferences.txt` until 2026-09-26, still read once), `Gazette Cache\`; CRLF on write. See `docs/windows.md`.
