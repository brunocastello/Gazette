...
- [x] Google News JS-redirect article links: best-effort decode (NewsProxy technique) **or** clear status-line fallback to feed summary.
  Superseded 2026-09-23 by Bruno's decision that Gazette reads the feed, not the site ("feed first, no fallback, photos kept", NetNewsWire's model). A Google News item shows Google's own entry, the list of outlets covering the story; nothing is decoded or fetched. The Mac failure that led here ("the connection failed while reading headers" on the page fetch) went away with the page fetch.
- [x] Articles come from the feed (commit `a467649` on main and the two after it): the parser keeps each item's body whole (content:encoded / <content>, else description / summary; a picture-only body counts), the extractor lays it out in fragment mode, and a refresh writes it with its pictures to "Text XXXXXXXX" beside the feed's cache. Opening an article reads it back from disk. Page fetch, Google decode, recall ring and the "Summary only" states removed. Host tests 785/785; Carbon build green.
- [x] Photo addresses have their entities decoded (`&#038;` → `&`); WordPress sizes were being lost. This was true of pages too.
- [x] README describes articles as the feed's own text.
- [ ] Try it on the Mac: refresh each feed once (texts are written by a refresh; until then an article shows its summary), then open a full-text feed (e.g. 9to5Mac, MacRumors), a Google News item and an NBC item.
- [ ] Decide whether to delete what nothing uses now: the Google News article-link resolver in `gazette_googlenews.c` (ArticleToken / Scan / BuildBody / ParseAnswer) with its host tests, and `GazetteFetchStartPost`. The topic maps stay (kept deliberately).
- [ ] Atom `type="xhtml"` content: the parser ends a capture at the first closing tag inside it, so such bodies come out cut short. Pre-existing; rare.
- [ ] `docs/release-notes.md` describes 0.1.0, which did fetch pages. Rewrite for the next release.
- [x] `probe.yml` now takes a feed URL and prints each item's pane text (`gh workflow run probe.yml -f feed=…`); `tls-probe.yml` probes one host through the vendored BearSSL; `build.yml -f net_debug=true` builds with transport detail on fetch failures (debug artifact shares the normal artifact's name; pick it by run).
...
- [x] README + `docs/release-notes.md` accurately describe what ships.
  Checked against the code on 2026-09-23 (three pictures, ⌘; Preferences, Hide/Show Photos, Gazette Cache, Top Stories starter feed, CarbonLib 1.1 + QuickTime, 8/4 MB). Since then the README says articles come from the feed; the release notes still describe 0.1.0 (see the release-notes item above).
